...
docker build --no-cache -f Dockerfile -t "augment-data-image" --build-arg TASK="${TASK}" --build-arg ARG="${ARG}" --build-arg MODEL_TYPE="${MODEL_TYPE}" .
docker run --name "augment-data-container" "augment-data-image"

# Source and destination dirs for all results
SOURCE_DIR="/usr/src/app/multilingual-bio-ner-and-el-msc-diss/scripts/data-aug/augment/${TASK}/out/${MODEL_TYPE}/${ARG}"
